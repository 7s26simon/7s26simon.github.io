#!/usr/bin/env python3
"""
update-writeups.py - Take vuln-tags.csv (output of tag-vulns.py),
apply manual overrides for the writeups that didn't auto-tag,
and rewrite writeups.html with:
...
def main():
    ...
    if n_select == 0:
        raise SystemExit("could not find <select id=cat-filter> in writeups.html")

    # Replace the <tbody>
    new_tbody = "            <tbody>\n                <!--\n                Auto-generated by _scripts/update-writeups.py.\n                Edit data-categories on each row to fix tagging.\n                Tokens must match <option value=\"...\"> in the select above.\n                -->\n" + tbody_html + "\n            </tbody>"
    ...
    if n_tbody == 0:
        raise SystemExit("could not find <tbody>...</tbody> in writeups.html")
